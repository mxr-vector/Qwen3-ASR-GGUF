...
struct llama_node_prof_data {
    int64_t t_start_node_us = 0;
    std::map<std::string, int64_t> node_times;
};

static bool llama_node_prof_cb(struct ggml_tensor * t, bool ask, void * user_data) {
    auto * data = (llama_node_prof_data *) user_data;
    if (ask) {
        data->t_start_node_us = ggml_time_us();
        return true;
    }
    int64_t dt = ggml_time_us() - data->t_start_node_us;
    if (t->name[0]) {
        data->node_times[t->name] += dt;
    }
    return true;
}
...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    if (can_reuse) {
        ...
        if (!graph_reuse_disable && ubatch.n_tokens > 1) {
             LLAMA_LOG_INFO("%s: reuse failed! n_tokens=%d, n_seqs=%d, n_outputs=%d, equal_seqs=%d\n",
                __func__, (int)ubatch.n_tokens, (int)ubatch.n_seqs, (int)n_outputs, (int)ubatch.equal_seqs());
             
             if (ubatch.n_tokens > 0) {
                 std::string seq_ids_str;
                 for (int k = 0; k < std::min((int)ubatch.n_tokens, 10); ++k) {
                     seq_ids_str += std::to_string(ubatch.seq_id[k][0]) + " ";
                 }
                 LLAMA_LOG_INFO("%s: first 10 tokens seq_ids: %s\n", __func__, seq_ids_str.c_str());
             }
        }
        res->reset();
        ...
        t_build_ms = (ggml_time_us() - t_build_start_us)/1000.0;
        ...
        t_alloc_ms = (ggml_time_us() - t_alloc_start_us)/1000.0;
    }

    // set the input data for the input tensors
    const int64_t t_set_start_us = ggml_time_us();
    res->set_inputs(&ubatch);
    double t_set_ms = (ggml_time_us() - t_set_start_us)/1000.0;

    llama_node_prof_data prof_data;
    if (ubatch.n_tokens > 1) {
        ggml_backend_sched_set_eval_callback(sched.get(), llama_node_prof_cb, &prof_data);
    }

#ifdef _WIN32
    PROCESS_MEMORY_COUNTERS mem_counters_start;
    if (ubatch.n_tokens > 1) {
        GetProcessMemoryInfo(GetCurrentProcess(), &mem_counters_start, sizeof(mem_counters_start));
        
        // --- CPU Warmup Segment ---
        // Spin for approx 20ms to boost frequency on Windows
        int64_t t_warmup_start = ggml_time_us();
        volatile int dummy = 0;
        while (ggml_time_us() - t_warmup_start < 20000) {
            dummy++;
        }
        // --------------------------
    }
    ...
    const int64_t t_comp_start_us = ggml_time_us();
    const auto status = graph_compute(res->get_gf(), ubatch.n_tokens > 1);
    double t_comp_ms = (ggml_time_us() - t_comp_start_us)/1000.0;

#ifdef _WIN32
    int64_t pf_delta = 0;
    if (ubatch.n_tokens > 1) {
        PROCESS_MEMORY_COUNTERS mem_counters_end;
        GetProcessMemoryInfo(GetCurrentProcess(), &mem_counters_end, sizeof(mem_counters_end));
        pf_delta = (int64_t)mem_counters_end.PageFaultCount - (int64_t)mem_counters_start.PageFaultCount;
    }
#endif
    ...
    if (ubatch.n_tokens > 1) {
        ggml_backend_sched_set_eval_callback(sched.get(), cparams.cb_eval, cparams.cb_eval_user_data);
#ifdef _WIN32
        LLAMA_LOG_INFO("%s: calculation page faults: %lld\n", __func__, (long long)pf_delta);
#endif

        std::vector<std::pair<std::string, int64_t>> sorted_nodes;
        for (const auto & it : prof_data.node_times) {
        ...
        LLAMA_LOG_INFO("%s: top 10 bottlenecks for prompt process (n_tokens=%d):\n", __func__, (int)ubatch.n_tokens);
        for (size_t i = 0; i < std::min((size_t)10, sorted_nodes.size()); ++i) {
            LLAMA_LOG_INFO("  - %32s: %7.2f ms\n", sorted_nodes[i].first.c_str(), sorted_nodes[i].second / 1000.0);
        }

        LLAMA_LOG_INFO("%s: reused=%d, build=%.2fms, alloc=%.2fms, set=%.2fms, compute=%.2fms, total=%.2fms\n",
            __func__, (int)reused, t_build_ms, t_alloc_ms, t_set_ms, t_comp_ms, (ggml_time_us() - t0_us)/1000.0);
    }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    const int64_t t_pre_balloc_us = ggml_time_us();
    if (!balloc->init(batch_inp, vocab, memory.get(), n_embd, n_seq_max, output_all)) {
        LLAMA_LOG_ERROR("%s: failed to initialize batch\n", __func__);
        return -1;
    }
    const int64_t t_balloc_init_us = ggml_time_us();
    ...
    const int64_t t_pre_sched_reserve_us = ggml_time_us();
    sched_reserve();
    const int64_t t_sched_reserve_us = ggml_time_us();

    bool did_optimize = false;

    // handle any pending shifts/copies
    const int64_t t_pre_memory_update_us = ggml_time_us();
    memory_update(false);
    const int64_t t_memory_update_us = ggml_time_us();

    if (n_tokens_all > 1) {
        LLAMA_LOG_INFO("%s: balloc=%.2fms, sched=%.2fms, memory=%.2fms\n",
            __func__, (t_balloc_init_us - t_pre_balloc_us)/1000.0,
            (t_sched_reserve_us - t_pre_sched_reserve_us)/1000.0,
            (t_memory_update_us - t_pre_memory_update_us)/1000.0);
    }

    int64_t t_compute_all_us = 0;
    ...
    do {
        ...
        ggml_status status;
        const int64_t t_start_ubatch_us = ggml_time_us();
        const auto * res = process_ubatch(ubatch, LLM_GRAPH_TYPE_DECODER, mctx.get(), status);
        t_compute_all_us += ggml_time_us() - t_start_ubatch_us;
    ...
    if (batch_inp.n_tokens > 1) { // Only log Prompt processing
        LLAMA_LOG_INFO("%s: n_tokens=%d, threads=%d/%d, overhead=%.2fms, balloc=%.2fms, sched=%.2fms, memory=%.2fms, compute=%.2fms, total=%.2fms\n",
            __func__, batch_inp.n_tokens, cparams.n_threads, cparams.n_threads_batch,
            (t_pre_balloc_us - t_decode_start_us)/1000.0,
            (t_balloc_init_us - t_pre_balloc_us)/1000.0,
            (t_sched_reserve_us - t_balloc_init_us)/1000.0,
            (t_memory_update_us - t_sched_reserve_us)/1000.0,
            t_compute_all_us/1000.0,
            (t_decode_end_us - t_decode_start_us)/1000.0);
    }
...
ggml_status llama_context::graph_compute(
    ...
    if (batched) {
        LLAMA_LOG_INFO("%s: threads=%d, tp=%p, tp_batch=%p, using_batched=%d\n", __func__, n_threads, (void*)threadpool, (void*)threadpool_batch, (int)batched);
    }
