...
def transcribe(
    ...
    config_table.add_row("加速设备", f"GPU:{'[green]ON[/green]' if use_gpu else '[red]OFF[/red]'} | Vulkan:{'[green]ON[/green]' if use_vulkan else '[red]OFF[/red]'}")
    ...
    with console.status("[bold yellow]正在初始化引擎，请稍候...[/bold yellow]") as status:
        try:
            ...
            console.print(f"  1. 尝试关闭 GPU 加速: 使用 [cyan]--no-gpu[/cyan]")
            console.print(f"  2. 尝试关闭 Vulkan 加速: 使用 [cyan]--no-vulkan[/cyan]")
            log_hint = Path(PROJ_DIR) / "logs" / "latest.log"
            console.print(f"  3. 如果问题仍然存在，请在 GitHub 提交 Issue 并附带 [cyan]{log_hint}[/cyan] 日志文件。")
            raise typer.Exit(code=1)
