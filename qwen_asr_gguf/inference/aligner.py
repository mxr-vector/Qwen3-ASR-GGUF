...
class AlignerProcessor:
    ...
    def fix_timestamps(self, data: np.ndarray) -> List[int]:
        ...
        while i < n:
            if not is_normal[i]:
                ...
                anomaly_count = j - i
                left_val = next(
                    (result[k] for k in range(i - 1, -1, -1) if is_normal[k]), None
                )
                ...
                if anomaly_count <= 2:
                    ...
                    if left_val is not None and right_val is not None:
                        ...
                        result[i:j] = [left_val] * anomaly_count
                    elif right_val is not None:
                        result[i:j] = [right_val] * anomaly_count
                i = j
            else:
                i += 1
    ...
    def reconcile(
        self, original_text: str, items: List[ForcedAlignItem]
        ...
        # 预先全量记录原始对齐结果
        raw_items_str = " | ".join(
            [f"'{it.text}'({it.start_time:.3f}-{it.end_time:.3f})" for it in items]
        )
        logger.debug(
            f"[Aligner] 原始对齐项列表 (总计 {len(items)} 个): [{raw_items_str}]"
        )

        if not items:
            return (
                [ForcedAlignItem(text=original_text, start_time=0.0, end_time=0.0)]
                if original_text
                else []
            )
        ...
        for i, item in enumerate(items):
            logger.debug(
                f"[Aligner] 处理原始项 idx={i}: '{item.text}' [{item.start_time:.3f}s - {item.end_time:.3f}s]"
            )
            # 搜索当前 item.text 在 original_text 中的位置 (跳过非保留字符)
            start_pos, end_pos = self._find_token_indices(
                original_text, item.text, curr_ptr
            )
            ...
            if start_pos != -1:
                ...
                if start_pos > curr_ptr:
                    ...
                    logger.debug(
                        f"[Aligner] 间隙项: '{gap_text}' at [{curr_ptr}:{start_pos}]"
                    )
                    reconciled.append(
                        ForcedAlignItem(
                            text=gap_text,
                ...
                logger.debug(
                    f"[Aligner] 匹配成功 idx={i}: '{item.text}' -> '{matched_text}' at [{start_pos}:{end_pos}]"
                )
                reconciled.append(
                    ForcedAlignItem(
                        text=matched_text,
                ...
                logger.warning(
                    f"[Aligner] 降级匹配 idx={i}: 无法在文本中找到 Token '{item.text}'，起始位置={curr_ptr}"
                )
                # 降级：若无法匹配则保持原样
        ...
        if curr_ptr < len(original_text):
            reconciled.append(
                ForcedAlignItem(
                    text=original_text[curr_ptr:], start_time=last_ts, end_time=last_ts
                )
            )
    ...
    def _find_token_indices(self, text: str, target: str, start_index: int):
        ...
        while i < txt_len:
            ...
            if ch == target[t_ptr]:
                ...
                if first_match != -1:
                    # 发生了回退
                    logger.debug(
                        f"[Aligner] Token 匹配回退: target='{target}' at text[{i}]='{ch}'，回退到 {first_match}"
                    )
                    i = first_match
                    first_match = -1
                    t_ptr = 0
        ...
        logger.debug(
            f"[Aligner] Token 查找失败: target='{target}', start_from={start_index}, context='{text[start_index:start_index+50]}...'"
        )
        return -1, -1
...
class QwenForcedAligner:
    ...
    def __init__(self, config: AlignerConfig):
        ...
        # 2. 加载对齐 LLM
        self.model = llama.LlamaModel(llm_gguf, n_gpu_layers=99 if use_gpu else 0)
        self.embedding_table = llama.get_token_embeddings_gguf(llm_gguf)
        self.ctx = llama.LlamaContext(
            self.model, n_ctx=config.n_ctx, n_batch=2048, embeddings=False
        )
    ...
    def align(
        ...
        pos_arr = np.concatenate(
            [pos_base, pos_base, pos_base, np.zeros(n_total, dtype=np.int32)]
        )
        batch = llama.LlamaBatch(n_total * 4, embd_dim=1024)
        batch.set_embd(full_embd, pos=pos_arr)
        for idx in ts_positions:
            batch.logits[idx] = 1  # 只计算 timestamp 处的 logits 以提速
        ...
        raw_ts = []
        for idx in ts_positions:
            logits_ptr = self.ctx.get_logits_ith(
                batch.n_tokens - (n_total - idx)
            )  # 对应 batch 中的索引
