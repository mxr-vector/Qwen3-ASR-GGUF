...
class QwenAudioEncoder:
    ...
    def __init__(self, frontend_path: str, backend_path: str, use_gpu: bool = True, pad_to: int = 30, verbose: bool = True, n_threads: int = 0):
        ...
        if use_gpu:
            # 优先选择 Linux 常见 GPU Provider，再兼容其他后端
            provider_priority = [
        ...
        # 预热处理
        if self.pad_to and self.pad_to > 0 and self.active_dml:
            ...
            # 动态形状模式下，预热一个短音频即可，无需固定长度 Padding
            if self.verbose: print(f"--- [Encoder] 正在预热 (动态形状模式)... ---")
